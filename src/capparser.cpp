...
{
CAPParser::CAPParser(const QByteArray &data, QObject *parent): m_xml(data)
{
    bool flag = false;
    ...
    if(!flag) {
        m_xml.raiseError(QObject::tr("Not a CAP XML"));
    }
}
        ...
        switch(tags[m_xml.name()]) {
            ...
            case Tags::STATUS:
            {
                ...
                if(elementText == QStringLiteral("Actual"))
                    ...
                    m_xml.raiseError(QObject::tr("Unknown status field"));
            ...
            case Tags::MSG_TYPE:
            {
                ...
                if(elementText == QStringLiteral("Alert"))
                    ...
                    m_xml.raiseError(QObject::tr("Unknown msgType field"));
            ...
            case Tags::SCOPE:
            {
                ...
                if(elementText == QStringLiteral("Public"))
                    ...
                    m_xml.raiseError(QObject::tr("Unknown scope field"));
...
AlertInfo CAPParser::parseInfo()
{
    ...
    if(m_xml.name() == QStringLiteral("info")) {
        while(!(m_xml.isEndElement() && m_xml.name() == QStringLiteral("info"))) {

            if(m_xml.isStartElement()) {
            switch(infoTags[m_xml.name()]) {
...
AlertInfo::Urgency getUrgency(const QString &s)
{
    ...
    else if(s == QStringLiteral("Unknown"))
