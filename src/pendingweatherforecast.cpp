...
void PendingWeatherForecastPrivate::applySunriseToForecast()
{
    ...
    auto getSymbolCodeDescription = [](bool isDay, const QString &symbolCode) { return isDay ? apiDescMap[symbolCode + "_day"].desc : apiDescMap[symbolCode + "_night"].desc; };

    auto getSymbolCodeIcon = [](bool isDay, const QString &symbolCode) { return isDay ? apiDescMap[symbolCode + "_day"].icon : apiDescMap[symbolCode + "_night"].icon; };
